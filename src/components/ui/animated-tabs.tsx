...
const AnimatedTabs = ({
  ...
  return (
    <div className={cn("w-full flex flex-col gap-y-1", className)}>
      <div className="flex gap-2 flex-wrap bg-white/5 dark:bg-black/5 p-1 rounded-xl">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={cn(
              "relative px-3 py-1.5 text-sm font-medium rounded-lg text-neutral-800 dark:text-neutral-200 outline-none transition-colors"
            )}
          >
            {activeTab === tab.id && (
              <motion.div
                layoutId="active-tab"
                className="absolute inset-0 bg-neutral-100 dark:bg-neutral-800 shadow-sm backdrop-blur-sm !rounded-lg"
      ...
      <div className="p-4 bg-white dark:bg-neutral-900 shadow-sm text-neutral-800 dark:text-neutral-200 rounded-xl border border-neutral-200 dark:border-neutral-800 min-h-60 h-full">
        {tabs.map(
          (tab) =>
            activeTab === tab.id && (
              <motion.div
